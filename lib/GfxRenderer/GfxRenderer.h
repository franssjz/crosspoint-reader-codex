...
class GfxRenderer {
 ...
 public:
  ...
  // Fork: drop an SD font's resident glyph tables when the heap is critically
  // low during a section build. Returns true if anything was released.
  ...
  // Fork aliases for promoteNextRefresh(): one-shot refresh-mode override
  // consumed by the next displayBuffer()/displayBufferAsync() call.
  void requestNextRefresh(const HalDisplay::RefreshMode mode) const {
    promotedRefresh_ = mode;
    promotedRefreshPending_ = true;
  }
  void clearNextRefreshOverride() const { promotedRefreshPending_ = false; }
  ...
  // Fork overloads: non-square icons, plus explicit black / inverted variants
  // used by the Lyra Carousel theme.
...
// RAII wrapper over beginStripTarget()/endStripTarget() (fork; used by the
// reader's tiled grayscale path in ReaderUtils.h).
