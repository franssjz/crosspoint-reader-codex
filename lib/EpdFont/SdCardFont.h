...
class SdCardFont {
 public:
  ...
  // Fork variant: reports presence separately from a zero advance (used by GfxRenderer fast paths).
  ...
  // Fork compatibility name for releaseResidentCaches() (NetworkMemory, PngSleepRenderer, GfxRenderer callers).
  void releaseForLowMemory() { releaseResidentCaches(); }
  ...
  // Fork compatibility: resolve a prewarmed mini glyph bitmap by its dataOffset. With the
  // contiguous mini arena this is simply miniData.bitmap + dataOffset (nullptr if no arena).
