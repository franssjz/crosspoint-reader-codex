...
parser.add_argument("--zopfli", action="store_true", help="Use lossless Zopfli raw DEFLATE and compact kerning (requires --compress).")
parser.add_argument("--compact-kerning", action="store_true", help="Use aligned split kerning maps and sparse rows where smaller.")
...
if args.zopfli and not args.compress:
    parser.error("--zopfli requires --compress")
...
# Buffer the generated header on the host so the same checked lossless encoder
# can optimize both new fonts and existing CPR headers without rerasterization.
header_stdout = sys.stdout
if args.zopfli or args.compact_kerning:
...
print("};")
if args.zopfli or args.compact_kerning:
    header, saved = optimize_header(sys.stdout.getvalue(), args.zopfli)
    sys.stdout = header_stdout
    print(header, end="")
    print(f"// Lossless encoding: {saved} flash bytes saved", file=sys.stderr)
