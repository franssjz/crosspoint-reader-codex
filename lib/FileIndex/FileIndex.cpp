...
constexpr const char* INDEX_DIR = "/.crosspoint/fileindex-cpr";
constexpr char MAGIC[4] = {'C', 'P', 'F', 'I'};
constexpr uint8_t INDEX_VERSION = 1;
constexpr size_t CHUNK_ENTRIES = 64;
constexpr size_t NAME_BUF_SIZE = FileIndex::MAX_NAME + 2;
...
bool FileIndex::open(const char* dirPath, AcceptFn accept, const void* context, uint32_t filterTag) {
  acceptContext = context;
  filterTag_ = filterTag;
  close();
  directoryReadFailed_ = false;
  if (!dirPath || !accept || strlen(dirPath) > UINT16_MAX) return false;
  ...
  // Create our regenerable directory before scanning so a first root listing
  // with hidden files enabled sees the same entries in both passes.
  if (!Storage.ensureDirectoryExists(INDEX_ROOT) || !Storage.ensureDirectoryExists(INDEX_DIR)) return false;
...
bool FileIndex::scanDirectory(const char* dirPath, AcceptFn accept, uint32_t& signature, uint32_t& dirs,
  ...
  for (auto file = root.openNextFile(); file; file = root.openNextFile()) {
    if (!file.getName(nameBuf.get(), FileIndex::MAX_NAME + 1)) {
      directoryReadFailed_ = true;
      file.close();
      root.close();
      return false;
    ...
    if (!accept(nameBuf.get(), isDir, acceptContext)) {
      file.close();
      continue;
    }
    ...
    file.close();
    if (static_cast<uint64_t>(dirs) + files > INT32_MAX) return false;
    maybeYield(yieldCounter);
  }
  if ((root.iterationFailed() || root.allocationFailed())) {
...
bool FileIndex::loadExisting(const char* dirPath, uint32_t signature, uint32_t dirs, uint32_t files) {
  snprintf(idxPath, sizeof(idxPath), "%s/%016llx-%08lx.idx", INDEX_DIR,
           static_cast<unsigned long long>(fnv1a64(dirPath)), static_cast<unsigned long>(filterTag_));
  ...
  bool valid = file.read(&h, sizeof(h)) == static_cast<int>(sizeof(h)) && memcmp(h.magic, MAGIC, sizeof(MAGIC)) == 0 &&
               ...
               h.fileCount == files && h.pathLen == strlen(dirPath);

  if (valid) {
    for (size_t pos = 0; valid && pos < h.pathLen;) {
      const size_t n = std::min<size_t>(NAME_BUF_SIZE, h.pathLen - pos);
      valid = file.read(nameBuf.get(), n) == static_cast<int>(n) && memcmp(nameBuf.get(), dirPath + pos, n) == 0;
      pos += n;
    }
  }
  ...
  if (valid) {
    ...
    valid = h.blobStart == sizeof(IndexHeader) + h.pathLen &&
            static_cast<uint64_t>(h.blobStart) + h.blobLen == h.offsetsStart && file.fileSize64() == expectedSize;
  }
...
void FileIndex::makeKey(RunRecord& rec, bool isDir, const char* name) const {
  ...
  FileIndexSort::naturalSortKey(name, rec.key + 1, sizeof(rec.key) - 1);
}
...
bool FileIndex::build(const char* dirPath, AcceptFn accept, uint32_t signature, uint32_t dirs, uint32_t files) {
  ...
  auto cleanupScratch = [&bs]() {
    if (bs.idxTmp) bs.idxTmp.close();
    if (bs.runsOut) bs.runsOut.close();
  ...
  if (ok) {
    ...
    if (!root || !root.isDirectory()) {
      ...
      for (auto file = root.openNextFile(); ok && file; file = root.openNextFile()) {
        ...
        if (static_cast<uint64_t>(blobStart) + bs.blobLen + sizeof(RecordHeader) + nameLen +
                (static_cast<uint64_t>(dirs) + files) * sizeof(uint32_t) >
            UINT32_MAX) {
          ok = false;
          file.close();
          break;
...
bool FileIndex::readNameAt(HalFile& file, uint32_t recordOffset, char* out, size_t cap) {
  ...
  if (rec.nameLen == 0 || rec.nameLen > MAX_NAME || rec.nameLen >= cap || rec.flags > 1) return false;
  const size_t n = rec.nameLen;
...
bool FileIndex::writeOffsets(BuildState& bs, uint32_t recordCount) {
  ...
  auto offsetComesBefore = [&](uint32_t left, uint32_t right) {
    ...
    const int cmp = FileIndexSort::naturalCompare(bs.nameA.get(), bs.nameB.get());
  ...
  auto sortSegmentByName = [&](size_t segmentLen) {
    ...
    for (size_t i = 1; ok && i < segmentLen; i++) {
      ...
      while (ok && j > 0) {
        ...
        const int cmp = ok ? FileIndexSort::naturalCompare(bs.nameB.get(), bs.nameA.get()) : 0;
...
bool FileIndex::entryAt(size_t row, Entry& out) {
  ...
  const auto fail = [this]() {
    // This file contains only derived names/offsets. Remove a damaged index so
    // the next visit rebuilds it instead of repeatedly showing the same error.
    close();
    Storage.remove(idxPath);
    return false;
  };
  uint32_t recordOffset = 0;
  if (!readOffsetForPhysIndex(row, recordOffset)) return fail();

  if (recordOffset < hdr.blobStart || static_cast<uint64_t>(recordOffset) + sizeof(RecordHeader) > hdr.offsetsStart)
    return fail();
  RecordHeader rec{};
  if (!idxFile.seek(recordOffset) || idxFile.read(&rec, sizeof(rec)) != static_cast<int>(sizeof(rec))) return fail();

  if (rec.nameLen == 0 || rec.nameLen > MAX_NAME || rec.flags > 1 ||
      static_cast<uint64_t>(recordOffset) + sizeof(rec) + rec.nameLen > hdr.offsetsStart)
    return fail();
  const size_t n = rec.nameLen;
  if (idxFile.read(out.name, n) != static_cast<int>(n)) return fail();
  out.name[n] = '\0';
  if (strlen(out.name) != n || strchr(out.name, '/') || strchr(out.name, '\\') || strcmp(out.name, ".") == 0 ||
      strcmp(out.name, "..") == 0)
    return fail();
...
size_t FileIndex::findRowByName(const char* name) {
  if (!opened || !name) return SIZE_MAX;
  ...
  while (pos < blobEnd) {
    ...
    if (rec.nameLen == 0 || rec.nameLen > MAX_NAME || rec.flags > 1 ||
        static_cast<uint64_t>(pos) + sizeof(rec) + rec.nameLen > blobEnd)
      return SIZE_MAX;
    const size_t n = rec.nameLen;
