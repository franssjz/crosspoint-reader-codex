...
class FileIndex {
 public:
  ...
  using AcceptFn = bool (*)(const char* name, bool isDir, const void* context);
  ...
  bool open(const char* dirPath, AcceptFn accept, const void* context = nullptr, uint32_t filterTag = 0);
  void close();
...
#pragma pack(pop)
  ...
  const void* acceptContext = nullptr;
  uint32_t filterTag_ = 0;
  HalFile idxFile;
  IndexHeader hdr{};
  bool opened = false;
