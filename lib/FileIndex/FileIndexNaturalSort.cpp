...
size_t naturalSortKey(const char* name, uint8_t* out, size_t cap) {
  ...
  while (*s && n < cap) {
    if (isdigit(static_cast<unsigned char>(*s))) {
      ...
      // A FAT UTF-8 name may contain more than 255 digits. Keep length
      // big-endian so the key preserves the complete numeric comparison.
      const size_t significant = len == 0 ? 1 : len;
      out[n++] = static_cast<uint8_t>(significant >> 8);
      if (n >= cap) break;
      out[n++] = static_cast<uint8_t>(significant);
      if (n >= cap) break;
      if (len == 0) {
        out[n++] = '0';
      } else {
