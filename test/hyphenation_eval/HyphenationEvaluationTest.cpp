...
// Fork (CPR-vCodex): Turkish patterns (measured 100% on the fork baseline).
TEST(HyphenationEval, Turkish) { runLanguageEval("turkish", "tr", "turkish_hyphenation_tests.txt", 99.00); }

// Fork (CPR-vCodex): Turkish dotted/dotless I casefolding must not change
// break positions between upper- and lower-case spellings of the same word.
TEST(HyphenationEval, TurkishCasefoldRegression) {
  const auto* hyphenator = getLanguageHyphenatorForPrimaryTag("tr");
  ASSERT_NE(hyphenator, nullptr) << "No Turkish hyphenator registered for casefold regression.";
  ...
  for (const auto& [source, expectedEquivalent] : equivalentWords) {
    ...
    EXPECT_EQ(sourceBreaks, equivalentBreaks)
        << "Turkish casefold regression: source=" << positionsToHyphenated(source, sourceBreaks)
