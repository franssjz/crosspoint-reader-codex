...
// Fork (CPR-vCodex): releases publish `<tag>.bin`; the parser must accept it
// and prefer it over a legacy `firmware.bin` asset in the same release.
TEST(ReleaseJsonParser, VcodexTagNamedAsset) {
  const char* json = R"({
  ...
  EXPECT_TRUE(p.foundTag());
  EXPECT_TRUE(p.foundFirmware());
  EXPECT_STREQ(p.getTagName(), "1.2.0.39-cpr-vcodex");
  EXPECT_STREQ(p.getFirmwareUrl(), "https://example.com/1.2.0.39-cpr-vcodex.bin");
  EXPECT_EQ(p.getFirmwareSize(), 6329000u);
}

TEST(ReleaseJsonParser, VcodexTagNamedAssetPreferredOverLegacy) {
  const char* json = R"({
      "tag_name": "1.2.0.39-cpr-vcodex",
      "assets": [
  ...
  EXPECT_TRUE(p.foundFirmware());
  EXPECT_STREQ(p.getFirmwareUrl(), "https://example.com/tagged.bin");
  EXPECT_EQ(p.getFirmwareSize(), 2u);
}

// Fork (CPR-vCodex): per-board asset naming. Each board's release binary is
// `<tag><suffix>.bin`, where <suffix> is derived from the board's legacy asset
// name ("firmware.bin" -> "", "firmware-x4pro.bin" -> "-x4pro"). The tag-named
// asset is preferred; the legacy per-board name is the fallback; assets that
// belong to another board are never selected.
TEST(ReleaseJsonParser, DefaultAssetNameTagMatchWinsWhenSeenFirst) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "1.6.0.1-cpr-vcodex.bin", "browser_download_url": "https://example.com/tagged.bin", "size": 2},
        {"name": "firmware.bin", "browser_download_url": "https://example.com/legacy.bin", "size": 1}
      ]
    })";
...
TEST(ReleaseJsonParser, DefaultAssetNameFallsBackToFirmwareBin) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "1.6.0.1-cpr-vcodex-x4pro.bin", "browser_download_url": "https://example.com/x4pro-tagged.bin", "size": 4},
        {"name": "firmware-x4pro.bin", "browser_download_url": "https://example.com/x4pro-legacy.bin", "size": 3},
        {"name": "firmware.bin", "browser_download_url": "https://example.com/legacy.bin", "size": 1}
      ]
    })";
  ...
  EXPECT_TRUE(p.foundFirmware());
  EXPECT_STREQ(p.getFirmwareUrl(), "https://example.com/legacy.bin");
  EXPECT_EQ(p.getFirmwareSize(), 1u);
}

TEST(ReleaseJsonParser, DefaultAssetNameIgnoresOtherBoardAssets) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "1.6.0.1-cpr-vcodex-x4pro.bin", "browser_download_url": "https://example.com/x4pro-tagged.bin", "size": 4},
...
TEST(ReleaseJsonParser, PerBoardAssetNamePrefersTagSuffixedBin) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "firmware.bin", "browser_download_url": "https://example.com/legacy.bin", "size": 1},
  ...
  ReleaseJsonParser p;
  p.setFirmwareAssetName("firmware-x4pro.bin");
  p.feed(json, strlen(json));

  EXPECT_TRUE(p.foundTag());
  EXPECT_TRUE(p.foundFirmware());
  EXPECT_STREQ(p.getFirmwareUrl(), "https://example.com/x4pro-tagged.bin");
  EXPECT_EQ(p.getFirmwareSize(), 4u);
}

TEST(ReleaseJsonParser, PerBoardAssetNameTagMatchWinsAfterLegacySeenFirst) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "firmware-x4pro.bin", "browser_download_url": "https://example.com/x4pro-legacy.bin", "size": 3},
...
TEST(ReleaseJsonParser, PerBoardAssetNameFallsBackToBoardFirmwareBin) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "firmware.bin", "browser_download_url": "https://example.com/legacy.bin", "size": 1},
  ...
  EXPECT_TRUE(p.foundFirmware());
  EXPECT_STREQ(p.getFirmwareUrl(), "https://example.com/x4pro-legacy.bin");
  EXPECT_EQ(p.getFirmwareSize(), 3u);
}

TEST(ReleaseJsonParser, PerBoardAssetNameIgnoresC3Assets) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "firmware.bin", "browser_download_url": "https://example.com/legacy.bin", "size": 1},
...
TEST(ReleaseJsonParser, PerBoardAssetNameSurvivesReset) {
  const char* json = R"({
      "tag_name": "1.6.0.1-cpr-vcodex",
      "assets": [
        {"name": "firmware.bin", "browser_download_url": "https://example.com/legacy.bin", "size": 1},
...
// Fork (CPR-vCodex): the browser auto-flash manifest parser used by the
// firmware update check.
TEST(FirmwareManifestJsonParser, ParsesManifest) {
  const char* json = R"({
      "name": "CPR-vCodex",
  ...
  EXPECT_STREQ(p.getVersion(), "1.3.0.9-cpr-vcodex");
