...
void FileBrowserActivity::loadFiles() {
  ...
  // Completed-book markers (reading stats), keyed by full path.
...
void FileBrowserActivity::rebuildRowItems() {
  ...
  for (size_t i = 0; i < files.size(); i++) {
    ...
    rowExtensions[i] = SETTINGS.hideFileExtension ? std::string() : getFileExtension(files[i]);
    fui::ListItem item;
    ...
    const bool completed = i < completedFileStates.size() && completedFileStates[i] != 0;
    item.icon = completed ? fui::bitmapFromIcon(icon_completed_badge_24) : listIconFor(UITheme::getFileIcon(files[i]));
...
void FileBrowserActivity::buildScreen(UiScreen& screen) {
  ...
  // Info band at the bottom: separator on top. Shows the selected entry's full
  // name (the list row already shows its beginning, so keep the suffix here:
  // books whose distinguishing text is near the end stay tellable apart), or
  // the current path when the folder is empty. Left-truncated either way.
  {
    ...
    const bool hasSelection = !files.empty() && nav.selected >= 0 && nav.selected < listCount();
    const char* pathStr = hasSelection ? files[nav.selected].c_str() : basepath.c_str();
