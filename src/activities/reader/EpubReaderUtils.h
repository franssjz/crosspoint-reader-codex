...
// The optional visible-text offset (upstream) is appended as 4 extra bytes so a
// re-paginated chapter can still resolve the exact page on reopen.
inline bool saveProgress(const Epub& epub, int spineIndex, int pageNumber, int pageCount,
  ...
  if (!ProgressFile::writeAtomic("ERS", epub.getCachePath(), data, dataSize)) {
    return false;
  }
