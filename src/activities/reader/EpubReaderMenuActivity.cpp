...
EpubReaderMenuActivity::EpubReaderMenuActivity(GfxRenderer& renderer, MappedInputManager& mappedInput,
  ...
  buildMenuItems(menuItems, hasFootnotes);
  buildMenuRowItems();
}
...
// construction; buildScreen() only touches the rows with a live value.
...
void EpubReaderMenuActivity::activateIndex(const int index) {
  ...
  if (selectedAction == MenuAction::NIGHT_MODE) {
    // Fork dark mode == upstream screenInverted (one persisted flag). Flip the
    // renderer right away so the menu itself repaints in the new mode; the
    // reader picks the change up from its settings snapshot when the menu
    // closes (applyReaderSettingsChanges) and re-renders the page.
...
void EpubReaderMenuActivity::buildScreen(UiScreen& screen) {
  ...
  props.count = static_cast<uint16_t>(std::min(menuItems.size(), MAX_MENU_ITEMS));
