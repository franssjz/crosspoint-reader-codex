...
def _apply_one(jpeg_dir, patch_path):
    ...
    if _git_apply_succeeds(jpeg_dir, patch_path, reverse=True):
        return
    # Fork: a libdep checkout may already carry one of the older, hand-applied
    # CrossPoint pMCU / DC-guard patch variants. Detect those instead of
    # failing `git apply --check` on a half-diverged jpeg.inl.
